# Plan de implementacion v2.1 — ETL IVR Pipeline

**Fecha:** 2026-05-07
**Version anterior:** `PLAN-IMPLEMENTACION-V2.md` (v2.0 — 66 tareas)
**Version actual:** v2.1 — **66 tareas**

## Cambios respecto a v2.0

| ID | Tarea | Cambio |
|---|---|---|
| C-01 | T-001 | Criterio de version: distingue sandbox 10.11.x de produccion 10.1.x |
| C-02 | T-015 | Reescribe bloque de festivos — todos retornan TRUE. Elimina NOTA Semana Santa |
| C-03 | T-016 | Corrige comentario SQL `-- 22` a `-- 23` para enero_2025 |
| C-04 | T-021 | Corrige `13 columnas` a `14 columnas` — `cargado_en` siempre estuvo |
| C-05 | T-018 | Corrige teorico de ratio: sin ajuste por festivos, Enero=23 dias |
| C-06 | T-019 | Reemplaza `CREATE OR REPLACE` por `DROP IF EXISTS + CREATE` |
| C-07 | Header | Actualiza fecha y version |
...
**Criterio sandbox:** Version 10.11.x aceptada (Ubuntu noble incluye 10.11.14).
**Criterio produccion:** Version 10.1.x confirmada (servidor del cliente).
En ambos casos: database = `ivr_legacy`, conexion sin error.
...
### T-015 — Verificar ivr_es_dia_semana *(MODIFICADA v2.1)*

**Contexto v2.1:** `ivr_es_dia_semana` = `DAYOFWEEK NOT IN (1,7)`.
Retorna TRUE para lunes-viernes, FALSE para sabado-domingo.
El IVR opera 7 dias incluyendo festivos — los datos confirman volumen
normal en todos los festivos nacionales. P-Semana-Santa: CERRADO.

```sql
-- Dias de semana y fin de semana
SELECT
    ivr_es_dia_semana('2025-01-06') AS lunes,    -- TRUE  (lunes)
    ivr_es_dia_semana('2025-01-07') AS martes,   -- TRUE  (martes)
    ivr_es_dia_semana('2025-01-04') AS sabado,   -- FALSE (sabado)
    ivr_es_dia_semana('2025-01-05') AS domingo;  -- FALSE (domingo)

-- Festivos nacionales en dia de semana: todos TRUE
-- El IVR opera y registra llamadas normales en estos dias
SELECT
    ivr_es_dia_semana('2025-01-01') AS anio_nuevo,    -- TRUE  (miercoles)
    ivr_es_dia_semana('2025-02-05') AS constitucion,  -- TRUE  (miercoles)
    ivr_es_dia_semana('2025-03-21') AS juarez,        -- TRUE  (viernes)
    ivr_es_dia_semana('2025-05-01') AS dia_trabajo,   -- TRUE  (jueves)
    ivr_es_dia_semana('2025-09-16') AS independencia, -- TRUE  (martes)
    ivr_es_dia_semana('2025-11-20') AS revolucion,    -- TRUE  (jueves)
    ivr_es_dia_semana('2025-12-25') AS navidad;       -- TRUE  (jueves)
```

**Criterio:** Lunes-viernes = TRUE. Sabado-domingo = FALSE.
Festivos en dia de semana = TRUE (IVR opera esos dias).
P-Semana-Santa: CERRADO — festivos no aplican para este IVR.
...
```sql
SELECT
    ivr_contar_dias_semana('2025-01-01', '2025-01-31') AS enero_2025, -- 23  (1 ene=mier, cuenta como dia de semana)
...
-- 2. Ratio razonable: dias de semana = 5/7 del tiempo ≈ 71.4%
-- Sin ajuste por festivos (ivr_es_dia_semana no excluye festivos)
SELECT
...
-- Si pct_entre_semana < 60% o > 85%: posible error en ivr_es_dia_semana
...
GROUP BY fecha
ORDER BY fecha;
-- Enero (23 dias lun-vie / 31 total = 74.2%), Feb (20/28 = 71.4%), Mar (21/31 = 67.7%)
-- (Nota: 1-ene=mier, 5-feb=mier, 21-mar=vie cuentan como dias de semana)
-- Tolerancia: +-5pp respecto al teorico
```
...
-- Version "rota" que siempre retorna TRUE
-- CREATE OR REPLACE no existe en MariaDB 10.1 — usar DROP + CREATE
...
**Criterio:** 14 columnas presentes incluyendo `llamadas_entre_semana`,
`llamadas_fines_semana` (v2) y `cargado_en`. 6 indices incluyendo `uk_grain`.
